...
/**
 * Build-tree-wide cache of transformed external artifact sets.
 *
 ...
 * This cache shares those sets across scopes. Keys are content-based: they incorporate the component,
 ...
 * parameter values) and its from/to attributes. Transform steps must therefore be isolated before a key is
 * computed. The capabilities and source variant id captured by the value are derived from the keyed component
 * and variant metadata, so they need no key ingredient of their own.
 *
 * <p>One ingredient is identity-based rather than content-based: the resolved source artifacts
 * ({@link ArtifactIdentities}). External artifacts are deduplicated by the build-tree-scoped per-repository
 * resolved-artifact caches, so scopes resolving the same artifact from the same repository see the same
 * {@link ResolvableArtifact} instances, while a same-GAV module served with different bytes by a different
 * repository yields different instances and must not share a transformed set. There is no content identity
 * available at this point that would not force artifact downloads, and identity is a stable and correct
 * discriminator for an in-memory, per-build-tree cache.
 *
...
public class SharedTransformedVariantCache {
    ...
    /**
     * A cache key for a transformed external variant. Mostly content-based, see the class documentation.
     */
